...
def test_a_cache_survives_parallel_writers():
    # the search cache got shredded once when two writers saved it at once. This pins
    # the writer down for many writers at a time; the torn file itself came from two
    # processes, which this cannot stage, so the real guard is the atomic replace.
    import threading
    ...
    json.loads(path.read_text(encoding="utf-8"))  # a torn write raises here
