...
def load_cache(path):
    """A cache is rebuildable: start over rather than crash on a damaged file."""
    try:
        return load(path, {})
...
def write_json(path, data, **kw):
    """Write through a temp file: a half-written cache crashes the next run."""
...
def pmap(fn, items, workers=8):
    # ponytail: threads, not asyncio - every call in here is a blocking urlopen
    items = list(items)
    if not items:
        return []
...
def http_get(host, path):
    """One kept-alive connection per host per thread: the handshake costs more than the query."""
    ...
    for attempt in (0, 1):
        ...
        try:
            ...
            if attempt or isinstance(e, urllib.error.HTTPError):
                raise  # a dead keep-alive is worth one silent retry, a real error is not
...
def lfm_wait():
    """One Last.fm slot every 0.25s; callers wait their turn, then request in parallel."""
    with _lfm_gate:
...
def lfm(**params):
    ...
    for attempt in (0, 1):
        ...
        try:
            ...
            if attempt:  # one retry: under load Last.fm drops a request now and then
                raise
    ...
    if todo and key and _lfm_fail[0] < 5:
        def fetch(n):
            ...
            try:
                ...
                _lfm_fail[0] += 1  # ponytail: racy count, it only gates a retry
                return n, None
...
def genre_name(gid):
    if not gid:  # no genre on Deezer: asking it for genre/None just burns a call
        return UNKNOWN
    if not _gname:
        # the Deezer genre list takes seconds to answer and never changes, so keep it
...
def find_artist(name, with_fans=False):
    ...
    got = cache[name]
    if isinstance(got, int):  # cached before the fan count was kept
        got = [got, 0]
    if with_fans:
...
def playable(cand, known_t, seen, rnd, listeners, target, window, ultra):
    ...
    for deeper in passes:
        ...
        for i in range(0, len(good), PLAYS_BATCH):
            ...
            # ponytail: warm the cache in parallel, then read it back in order
...
def obscure_pool(seeds, prefs, rnd, page=ULTRA_TAG_PAGE, want=ULTRA_POOL):
    def tail_of(tag):
        # a small tag runs out of pages: fall back to its own tail rather than nothing
    ...
    rnd.shuffle(names)
    names = names[:want]
    # Deezer's fan count is one fast call and rules out the famous ones before
    # we spend two rate-limited Last.fm calls each on them
...
def pick(avoid_bucket=None):
    ...
    def attempt(window):
        ...
        for strict, seed in rounds:
            ...
            # Deezer answers in about a second, so warm what the filters need in parallel
...
def tag_artists(tag, limit=30, page=1):
    # Last.fm can take ten seconds to serve one of these and a tag chart barely
    # moves, so it is worth keeping on disk between runs
    cache = tag_cache()
...
def settings_data():
    ...
    return {"source": source, "new": prefs.get("new", False), "extra": extra,
            ...
            # the page posts back what it got: without these two, saving wipes them
